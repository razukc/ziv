...
export default function SimulationPanel({ pipeline, robot, seed, onClose, onNewScenario }: SimulationPanelProps) {
  const plan = useMemo(() => planDryRun(pipeline, robot, seed), [pipeline, robot, seed]);
  ...
  // Auto-run on mount and whenever the scenario seed changes.
  useEffect(() => { resetRun(); /* eslint-disable-next-line react-hooks/exhaustive-deps */ }, [seed, plan]);
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
            ...
            <span data-testid="dryrun-seed" title="same plan + same seed = same outcome (deterministic)" style={{ fontSize: "10px", color: "var(--text-faint)", fontFamily: "var(--font-mono)", background: "var(--line)", padding: "1px 6px", borderRadius: "3px", cursor: "help" }}>
              seed 0x{seed.toString(16).padStart(4, "0")}
            </span>
        ...
        {finished ? (
          <>
            <span data-testid="dryrun-elapsed" style={{ color: "var(--text-faint)" }}>
              completed in {fmtReplay(displayMs)} · ${costIncurred.toFixed(2)} incurred
            </span>
