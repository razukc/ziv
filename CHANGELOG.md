...
- **The QEMU boot check now reads the derived fixture** — the rung-2 differ (`tools/qemu_timeline.py`, until now only *named* in the ladder doc and the host bench) exists: its expected side is `build_ziv_demo.demo_fixture_lines()` — the same `DEMO_STAGES` derivation that generates the app's `k_demo_sequence[]` and `k_demo_expected[]` — cross-checked against the `k_demo_expected[]` strings parsed from the generated `.c`, so a stale generated file fails with exit 2 and the heal command instead of comparing against wrong lines. The differ compares two `HAP` logs (event order, modes, payloads, dot masks, buzz durations, beat indices exactly; timestamps and gaps within ±10 ms wall-clock tolerance) and ignores boot-banner chatter. The bench side of the future CI boot job is `host_demo --hap-log` (same binary, same sequencer stream the C suite asserts), and `run_ziv_tests.py` proves the whole plumbing on every run: after the 49 C checks it emits the bench HAP log and diffs it against the derived fixture — the QEMU job will reuse the identical invocation with the boot console capture as the second log. Hermetic pytest coverage: self-check, golden pass with jitter + chatter, three real drift catches (mask, timestamp beyond tolerance, missing event), and the stale-fixture refusal. CI step renamed; ladder doc rung-2 status moved from "speced" to "differ shipped".
...
- **The `qemu-boot` CI job exists as a skeleton — the IDF install is its only missing piece** — the rung-2 boot check is now a real job shape in CI (`continue-on-error` until the first green boot, per the promote-a-check discipline): checkout, deps, and the bench side run for real on every push (`run_ziv_tests.py` emits `bench_hap.log` and re-proves the differ end to end), while the marked ESP-IDF + Espressif QEMU install step is the single designated gap — the boot-capture step (`timeout 60 idf.py qemu > qemu.log`, ⚠ verified at install time) and the differ step (`tools/qemu_timeline.py bench_hap.log qemu.log`, the exact invocation the host suite already proves) are gated on the install step's `available` output and light up when it lands, with nothing else to change. Ladder doc rung-2 status and CI-shape updated to match.
